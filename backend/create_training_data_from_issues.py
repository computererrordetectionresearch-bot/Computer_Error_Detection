"""
Create training data CSV from real-world user issues.
This will be used to retrain the error detection model.
"""
...
# Real-world issues with expected error types (same as test file)
REAL_WORLD_ISSUES = {
    ...
    "PC start beep sound": "RAM Upgrade",
...
def create_training_csv():
    """Create training data CSV from real-world issues."""
    print("=" * 80)
    print("CREATING TRAINING DATA FROM REAL-WORLD ISSUES")
    print("=" * 80)
    
    # Create DataFrame
    rows = []
    for issue, error_type in REAL_WORLD_ISSUES.items():
        rows.append({
            'user_text': issue,
            'error_type': error_type,
            'component_label': error_type,  # For compatibility
            'source': 'real_world'
        })
    ...
    # Save to CSV
    output_file = DATA_DIR / "real_world_error_training_data.csv"
